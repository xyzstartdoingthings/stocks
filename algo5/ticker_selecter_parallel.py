...
variables = {"sma_length": [150, 200], "rsi_period": [8,10,12], "long_range": [20,30,40], 
             ...
             "midline_sell": [60,50], "day_max": [11], "percent_trend": [0.5,0.8], "percent_trend2":[0.8,1]}
...
def optimizer_parallel(algo, ticker, variables):
    result = optimizer(algo, ticker, variables, max_dd=0.15, wl_ratio=0.7, eval=eval_open)
    if result:
        return result
    else:
        return []
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=24) as executor:
        ...
        #finer
        futures = [executor.submit(optimizer_parallel, algo4, ticker, variables)
