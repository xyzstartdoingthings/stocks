...
def algo2(ticker, atr_len=13, atr_changeATR=True, macd_fastLen=13, macd_slowLen=34, macd_src="close", macd_signalSmooth=9, macd_oscMA="EMA", macd_sigMA="EMA", macd_peakLen=5, gain_ratio=2, loss_ratio=1.5, peak2_len=20, peak3_len=40):
    ...
    #macd
    stock = pine_MACD(macd_fastLen, macd_slowLen, macd_src, macd_signalSmooth, macd_oscMA, macd_sigMA, stock)
            ...
            if short_mark == 1:
                if i >= sb_date:
                    if stock.loc[i, "MACD_Hist"] < stock.loc[i-1, "MACD_Hist"]:
                        stock.loc[i, "order"] = -1
                        stoploss = stock.loc[i, "close"] + stock.loc[i, "atr"] * loss_ratio
                        stopgain = stock.loc[i, "close"] - gain_ratio * stock.loc[i, "atr"]
                        short_mark=0
                        sb_date = 0
                else:
                    continue
                ...
                peak["close"] = max(stock.loc[i,"close"], stock.loc[i-1,"close"])
                ...
                trough["close"] = max(stock.loc[i,"close"], stock.loc[i-1,"close"])
